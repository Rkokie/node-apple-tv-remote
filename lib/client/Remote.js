...
var Remote = function (host, options) {
    ...
    this._cmd = function (cmd, params) {
        var url = '' + cmd + '?sessionId=' + me.sessionId;
        if (params) {
...
Remote.prototype.sendButtonPress = function (button) {
    ...
    if (button !== 'object' || !buttons.hasOwnProperty(button.name))
        throw new Error('Invalid Button Type! Please use BUTTONS for available types');
    ...
    var options = {
        ...
        uri: this._cmd('/ctrl-int/1/controlpromptentry', {'prompt-id': this.promptId, 'session-id': 1})
    };
